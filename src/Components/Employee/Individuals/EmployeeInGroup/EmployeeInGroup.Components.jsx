...
import EmployeeList from "../EmployeeList/employeeList.Components";
...
// const EmployeeListLazy = React.lazy(() =>
//   import("../EmployeeList/employeeList.Components")
// );
const EmployeeListLazy = withSpinner(EmployeeList);
...
const EmployeeInGroup = ({
  ...
  selectIsFetchingGroup,
  selectIsFetchingEmployeeInfo,
}) => {
  const [statusError, setStatusError] = useState(
    "Please Add Your Employee in the Group"
  );
  ...
  return (
    ...
    >
      <Box sx={{ gridArea: "directory" }}>
        <Typography sx={{ display: "flex", ml: 5 }}>
          <Link to="/grps">
            {" "}
            <Typography sx={{ color: "black" }}>Groups</Typography>{" "}
          </Link>{" "}
          / Group {id}
        </Typography>
      </Box>
      <Box sx={{ gridArea: "switch" }}>
        ...
        >
          <ToggleButton
            value="Employee"
            onClick={() => dispatch(OptionBetweenGroupAndTask(true))}
            sx={{ border: 0 }}
          >
            Employee
          </ToggleButton>
          <ToggleButton
            value="Task"
            onClick={() => dispatch(OptionBetweenGroupAndTask(false))}
            sx={{ border: 0 }}
          >
            Task
          </ToggleButton>
      ...
      {optionGroupAndTask ? (
        <Grid
          container
          sx={{ gridArea: "container-list-addEmp" }}
          spacing={3}
            ...
            {employee_list.length === 0 ? (
              <Grid item>
                {employeeListTemp.length === 0 ? (
                  ...
                  <Box>
                    <AddEmployeeListTemp
                      employeeListTemp={employeeListTemp}
                      idGroup={idGroup}
                    />
                  </Box>
                )}
                ...
                {!triggerButtonOpenAndCloseRateCard ? (
                  ...
                  >
                    <RateCard
                      ...
                      displayName={
                        employeeInfo[selectEmployeeIDForRateCard].displayName
                      }
                      idGroup={idGroup}
...
const mapStateToProps = (state, ownProps) => ({
  ...
  selectIsFetchingGroup: selectIsFetchingGroup(state),
  selectIsFetchingEmployeeInfo: selectIsFetchingEmployeeInfo(state),
});
